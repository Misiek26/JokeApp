...
<a href="https://sv443.net/privacypolicy/en" target="_blank">View the privacy policy by clicking here.</a>
...
var idRanges = {};
var maxJokeIdRange = parseInt("1368");
...
function onLoad()
{
    ...
    console.log("%cJokeAPI%cDocumentation (v2.3.2)  -  © Copyright Sv443 Network 2018-2020", "color: #b05ffc; background-color: black; padding: 5px; padding-right: 0; font-size: 1.2em;", "color: white; background-color: black; padding: 5px; font-size: 1.2em;");
    ...
    sMenu.new("restartPrompt", "Restart JokeAPI", rsIHTML, 40, 30, true, true, true);
    ...
    try
    {
        var fileFormats = JSON.parse('["JSON","XML","YAML","TXT"]');
    ...
    infoXhr.onreadystatechange = function() {
        if(infoXhr.readyState == 4 && infoXhr.status < 300)
        {
            ...
            for(var i = 0; i < idrKeys.length; i++)
            {
                ...
                try
                {
                    console.info("JokeAPI is serving " + (respJSON.jokes.idRange[idrKey][1] + 1) + " jokes from language \"" + idrKey + "\"");
                }
                catch(err)
                {
                    void(err);
...
function reRender(langChanged)
{
    ...
    if(langChanged === true)
    {
        ...
        {
            gebid("idRangeInputTo").max = parseInt("1368");

            maxJokeIdRange = parseInt("1368");
        }
    }
...
function resetTryItForm(confirmation)
{
    ...
    gebid("idRangeInputTo").value = parseInt("1368");
...
function createJoke(){
    let firsPath = document.getElementById("urlBuilderUrl").value;
    let lastPath = firsPath.replace('https://v2.jokeapi.dev', 'http://localhost:8080/api/external');

    $( "#jokeContent" ).load( lastPath, function( response, status, xhr ) {
      if ( status == "error" ) {
